...
if (!SUPABASE_URL || !SUPABASE_SECRET_KEY) {
  console.warn('[Hotel Cortez] AVISO: SUPABASE_URL e/ou SUPABASE_SECRET_KEY não definidas. Sincronização com Supabase desabilitada.');
}
...
app.get('/api/health', async (_req: Request, res: Response) => {
  if (!supabaseAdmin) {
    return res.status(503).json({
      ...
      eventTicketsTable: false,
      couponsTable: false,
      guestListTable: false,
      contactsTable: false,
    });
  }

  const [ticketsCheck, couponsCheck, guestsCheck, contactsCheck] = await Promise.all([
    supabaseAdmin.from('event_tickets').select('id').limit(1),
    supabaseAdmin.from('coupons').select('id').limit(1),
    supabaseAdmin.from('guest_list').select('id').limit(1),
    supabaseAdmin.from('contacts').select('id').limit(1),
  ]);
  return res.status(ticketsCheck.error ? 503 : 200).json({
    status: ticketsCheck.error ? 'degraded' : 'ok',
    supabaseConfigured: true,
    eventTicketsTable: !ticketsCheck.error,
    couponsTable: !couponsCheck.error,
    guestListTable: !guestsCheck.error,
    contactsTable: !contactsCheck.error,
    databaseError: ticketsCheck.error ? ticketsCheck.error.code : undefined,
  });
});
...
function generateSessionToken(): string {
  if (!ADMIN_SESSION_SECRET) throw new Error('Autenticação administrativa não configurada.');
...
interface PersistedTicketResult {
  data: any | null;
  error: any | null;
  token: string | null;
...
async function createPersistedTicket(name: string, phone: string): Promise<PersistedTicketResult> {
  if (!supabaseAdmin) return { data: null, error: new Error('Supabase indisponível.'), token: null, createdAt: null };
  ...
  for (let attempt = 0; attempt < 5; attempt += 1) {
    ...
    if (error?.code !== '23505') {
      ...
      return { data: null, error, token: null, createdAt: null };
    }
  }

  return { data: null, error: new Error('Não foi possível gerar um código único.'), token: null, createdAt: null };
}
...
app.post('/api/guestlist', publicWriteLimiter, async (req: Request, res: Response) => {
  ...
  if (!supabaseAdmin) return res.status(503).json({ error: 'Persistência da lista VIP indisponível.' });
  const { data: persistedGuest, error } = await supabaseAdmin.from('guest_list').insert([
    {
      name: newEntry.name,
  ...
  if (error || !persistedGuest) {
    logSupabaseError('POST guest_list insert', error);
    return res.status(500).json({ error: 'Não foi possível salvar o nome na Lista VIP.' });
  }

  newEntry.id = persistedGuest.id;
...
app.post('/api/admin/login', adminLoginLimiter, (req: Request, res: Response) => {
  ...
  if (!ADMIN_USER || !ADMIN_SECRET || !ADMIN_SESSION_SECRET) {
    return res.status(503).json({ error: 'Autenticação administrativa não configurada no servidor.' });
  }
...
// The stateless token is removed from sessionStorage by the frontend on logout.
...
app.get('/api/admin/metrics', requireAdminAuth, async (_req: Request, res: Response) => {
  if (supabaseAdmin) {
    ...
    if (couponsError) logSupabaseError('GET admin metrics coupons', couponsError);
...
app.post('/api/admin/tickets/create', requireAdminAuth, async (req: Request, res: Response) => {
  ...
  if (cleanName.length < 2) return res.status(400).json({ error: 'Informe um nome válido.' });
  if (sanitizePhone(cleanPhone).length < 10) return res.status(400).json({ error: 'Informe um celular válido com DDD.' });
  ...
  const created = await createPersistedTicket(cleanName, cleanPhone);
  const persistedTicket = created.data;
  const error = created.error;
  const token = created.token;
  const createdAt = created.createdAt;
  if (error || !persistedTicket || !token || !createdAt) {
...
app.post('/api/contact', publicWriteLimiter, async (req: Request, res: Response) => {
  ...
  if (!supabaseAdmin) return res.status(503).json({ error: 'Persistência de mensagens indisponível.' });
  ...
  if (error) {
    logSupabaseError('POST contacts insert', error);
