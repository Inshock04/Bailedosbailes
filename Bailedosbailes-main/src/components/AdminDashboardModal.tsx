...
export const AdminDashboardModal: React.FC<AdminDashboardModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleDeleteUser = async (ticketId: string) => {
    try {
      ...
      if (!response.ok) throw new Error(data?.error || 'Não foi possível cancelar o ingresso.');
      setMetrics((prev: any) => ({
        ...prev,
      ...
      setAuthError(err.message || 'Não foi possível cancelar o ingresso.');
      audioManager.playError();
    }
  };
  ...
  const handleSaveEdit = async (ticketId: string) => {
    ...
    try {
      ...
      setAuthError(err.message || 'Não foi possível salvar as alterações.');
      audioManager.playError();
    } finally {
      setSavingEdit(false);
    }
