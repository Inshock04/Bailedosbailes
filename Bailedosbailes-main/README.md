...
Cadastre estas variáveis em **Settings > Environment Variables**, habilite-as para o ambiente correto e faça um novo deploy:

```env
SUPABASE_URL=https://upijucscuvnxeqdetrhmv.supabase.co
SUPABASE_SECRET_KEY=<chave sb_secret rotacionada>
ADMIN_USER=<login administrativo>
ADMIN_KEY=<senha administrativa forte>
ADMIN_SESSION_SECRET=<segredo aleatório longo e diferente da senha>
```

Nunca use o prefixo `VITE_` em uma variável secreta. O frontend não precisa da chave do Supabase: toda persistência passa por `/api`.

Depois do deploy, consulte `/api/health`. `supabaseConfigured` e `eventTicketsTable` devem ser `true`. O endpoint também informa separadamente a disponibilidade de `coupons`, `guest_list` e `contacts`.
...
Antes de publicar a funcionalidade do Oráculo, execute todo o `supabase/schema.sql` no SQL Editor do projeto Supabase configurado. Sem `public.coupons`, o Oráculo retorna erro de persistência, mas a listagem de ingressos administrativos continua disponível. A migration adiciona `device_id`, `redeemed_at` e o índice único por dispositivo sem apagar registros existentes.
