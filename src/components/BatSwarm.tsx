...
export const BatSwarm: React.FC = () => {
  ...
  useGSAP(() => {
    if (isMobile) {
      // Mobile: Only 2 bats with simpler motion (no sine-wave oscillation)
      gsap.fromTo('.flying-bat-1',
        { x: '110vw', y: '18vh', scale: 0.8, opacity: 0 },
        { x: '-20vw', y: '22vh', opacity: 0.9, duration: 12, repeat: -1, ease: 'none', repeatDelay: 4 }
      );

      gsap.fromTo('.flying-bat-2',
        { x: '-15vw', y: '10vh', scale: 1.1, opacity: 0 },
        { x: '115vw', y: '35vh', opacity: 0.85, duration: 14, repeat: -1, ease: 'none', delay: 5, repeatDelay: 6 }
      );
    } else {
      // Desktop: Full 5-bat experience with sine-wave altitude oscillation
